class ApiConstants {
  static String baseUrl = 'https://gih-pid084.herokuapp.com';
  static String login = '/login';
  static String stationDetails = '/station/info/';
  static String feedback = '/feedbacks/';
  static String generateQR = '/station/generateQR/';
  static String feedbackinfo = '/feedbacks/info/';
  static String chartRoute = '/chart/';
  static String stationRoute = '/station/';
  static String totalFeedbackStation = '/total/station/';
  static String totalFeedbackSubdivision = '/total/subdivision/';
  static String totalFeedbackDistrict = '/total/district/';
  static String averageForStation = 'average/station/';
  static String averageForSubdivision = 'average/subdivision/';
  static String averageForDistrict = 'average/district/';
  static String frequencyForStation = '/frequency/stations/';
  static String staton = '/list/stations/';
  static String generateExcel = '/station/generateExcel/';

}
